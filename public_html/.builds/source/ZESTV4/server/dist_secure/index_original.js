...
const csrfProtection = csurf({
  cookie: {
    httpOnly: true,
    secure: true, // Forçado para HTTPS na Hostinger
    sameSite: 'lax', // Permite compartilhamento entre subdomínios (api. -> principal)
    domain: '.zestmenu.com.br' // Crucial: Ponto no início compartilha cookie entre subdomínios
  }
});
...
app.put('/api/store/:id', authenticateToken, csrfProtection, nonceMiddleware, async (req, res) => {
  ...
  const { nome, telefone, cnpj, email, endereco, scheduleStr, promo_banner } = StoreUpdateDTO;

  try {
    const query = `
      UPDATE stores 
      SET nome = ?, telefone = ?, cnpj = ?, email = ?, endereco = ?, schedule = ?, promo_banner = ?
      WHERE id = ?
    `;
    ...
    const [result] = await pool.query(query, [
      nome, encTelefone, encCnpj, email, endereco, scheduleStr, promo_banner, storeId
    ]);
