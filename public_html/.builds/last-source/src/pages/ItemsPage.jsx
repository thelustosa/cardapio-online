...
const ItemsPage = ({ setIsDirty }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const parsedItemData = itemData.map(item => ({
          ...item,
          image: item.image && (item.image.startsWith('[') || item.image.startsWith('{'))
  ...
  const handleEditItemClick = (item) => {
    ...
    // Media previews
    const images = typeof item.image === 'string' && (item.image.startsWith('[') || item.image.startsWith('{'))
      ? JSON.parse(item.image)
  ...
  const handleCreateItem = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const parsedItem = {
          ...savedItem,
          image: savedItem.image && (savedItem.image.startsWith('[') || savedItem.image.startsWith('{')) 
            ? JSON.parse(savedItem.image) 
            : savedItem.image
        };
    ...
    >
      <header className={styles.itemsHeader}>
        ...
        <div className={styles.headerActions}>
          ...
          <button className={styles.secondaryBtn} onClick={() => setIsDrawerOpen(true)}>
            Novo Menu
          </button>
          <button className={styles.primaryBtn} onClick={() => setIsItemDrawerOpen(true)}>
            <Plus size={18} />
            Novo Item
          </button>
        </div>
              ...
              <div className={styles.drawerBody}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setSelectedItemMenu(e.target.value);
                      setSelectedItemSubCategory('');
                    }}
                  >
                ...
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    disabled={!selectedItemMenu}
                  >
                    <option value="">Selecione a categoria</option>
                    {(() => {
                      const menuObj = categories.find(c => c.name === selectedItemMenu);
                      if (menuObj && menuObj.subcategories && menuObj.subcategories.length > 0) {
                        return menuObj.subcategories.map((sub, i) => (
                          <option key={i} value={sub}>{sub}</option>
                        ));
                      }
                      if (selectedItemMenu) {
                        return <option value="" disabled>Lembre de adicionar Categorias no Menu '{selectedItemMenu}'.</option>;
                      }
                      return null;
                    })()}
                  </select>
                ...
                <div className={styles.formGroup}>
                  ...
                  <div style={{ marginTop: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <input type="checkbox" id="nullKitchen" onChange={(e) => e.target.checked && setItemKitchenStation('')} />
                    <label htmlFor="nullKitchen" style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>Manter campo vazio</label>
                  </div>
                </div>
